import ProjectTechnologies from "@/atomic-design/Molecules/ProjectTechnologies/ProjectTechnologies";
...
const Project = ({project}: ProjectProps) => {
  const {title, description, links, imagePath, technologies, launchDate} =
    project;
  return (
    <div
      className="pb-9 flex w-full gap-6 max-w-[936px]
    ...
    >
      <img
        src={imagePath}
        alt="project-image"
        className="w-[360px] pb-2 min-h-36 object-cover
        ...
        <div className="flex gap-2 pb-2 justify-start w-full">
          {links.map((link, index) => (
            <a
              ...
              href={link.url}
              target="_blank"
              className="text-sm italic font-semibold underline"
            >
              {link.title}
            </a>
          ))}
